...
export const Icon: FC<IconProps> = ({
  ...
  const ParsedIcon: ParsedIcon = {
    ...
    icon: Object.keys(evicons).includes(name)
      ? evicons[name as EveotIconName]
      : tbicons[name as TablerIconName],
  };
  ...
  return (
    <div className={`ev-icon ${className ?? ""}`} onClick={onClick} {...props}>
      {ParsedIcon.type === "eveot" ? (
        <ParsedIcon.icon width={size} height={size} color={currentColor} />
      ) : (
        <ParsedIcon.icon size={size} color={currentColor} />
      )}
    </div>
  );
};
